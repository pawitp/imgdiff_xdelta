...
unsigned char* MakePatch(ImageChunk* src, ImageChunk* tgt, size_t* size) {
  ...
  char ps1[] = "/tmp/imgdiff-patch-XXXXXX";
  mkstemp(ps1);

  char ps2[] = "/tmp/imgdiff-patch-XXXXXX";
  mkstemp(ps2);

  FILE* f1 = fopen(ps1, "wb");
  if (f1 == NULL) {
    printf("failed to open ps1 %s: %s\n", ps1, strerror(errno));
    return NULL;
  }
  if (fwrite(src->data, 1, src->len, f1) != src->len) {
    printf("failed to write ps1 %s: %s\n", ps1, strerror(errno));
    return NULL;
  }
  fclose(f1);

  FILE* f2 = fopen(ps2, "wb");
  if (f2 == NULL) {
    printf("failed to open ps2 %s: %s\n", ps2, strerror(errno));
    return NULL;
  }
  if (fwrite(tgt->data, 1, tgt->len, f2) != tgt->len) {
    printf("failed to write ps1 %s: %s\n", ps2, strerror(errno));
    return NULL;
  }
  fclose(f2);
  ...
  char command[200];
  sprintf(command, "xdelta3 -e -f -s %s %s %s", ps1, ps2, ptemp);
  printf("Exec: %s\n", command);
...
void DumpChunks(ImageChunk* chunks, int num_chunks) {
    int i;
    for (i = 0; i < num_chunks; ++i) {
        printf("chunk %d: type %d start %zd len %zd\n",
